...
const mapper = [
    ...
    ['°F|fahrenheit|f|degf', '°C', (val) => { return (val - 32) * 5 / 9; }],
    ['K|kelvin|degk', '°C', (val) => { return val - 272.15; }],
    ['rankine|degR', '°C', (val) => { return (val - 491.67) * 5 / 9; }],
 ...
 */
function supportedUnits(filter) {
 ...
 */
function toHumanReadable([val, format]) {
    if (!Number.isFinite(val) || !format)
        return '';
